import { GraphqlSchemaCompacter, GraphqlSchemaCompacterOptions } from "../../contracts/graphql-schema-compacter";
    ...
    async (url: string, options?: GraphqlSchemaCompacterOptions): Promise<string> => {
        ...
        if (queryTypeName) {
            ...
            if (queryType?.fields?.length) {
                sections.push(`# Queries\n${renderRootFields(queryType.fields)}`);
            }
        }
        ...
        if (mutationTypeName) {
            ...
            if (mutationType?.fields?.length) {
                sections.push(`# Mutations\n${renderRootFields(mutationType.fields)}`);
            }
        }
            ...
            switch (t.kind) {
                case "OBJECT":
                    if (t.fields?.length) {
                        ...
                        const ownFields =
                            inherited.size > 0 ? t.fields.filter((f) => !inherited.has(f.name)) : t.fields;
