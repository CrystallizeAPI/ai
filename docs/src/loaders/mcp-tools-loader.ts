...
async function getToolRegistry(): Promise<Map<string, string>> {
    ...
    const registryMatch = content.match(/export const toolRegistry\s*=\s*\{([\s\S]*?)\}\s*as\s*const/);
    ...
    const importRegex = /import\s*\{[^}]*\}\s*from\s*["']\.\/mcp\/tools\/([^"']+)["']/g;
    ...
    for (const [toolName, wrapperName] of mapping) {
        ...
        const createFnMatch = content.match(new RegExp(`${wrapperName}:\\s*asFunction\\((\\w+)\\)`));
        if (createFnMatch) {
...
function extractInputSchema(source: string): ZodField[] {
    ...
    const fieldRegex = /(\w+):\s*z\b([\s\S]*?)\.describe\(\s*(?:`([\s\S]*?)`|"([\s\S]*?)"|'([\s\S]*?)')\s*,?\s*\)/g;
    let match;
